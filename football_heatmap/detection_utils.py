...
def draw_track_frame_dsort(track_detection,detection_frame):
    "Draw detections of deepsort tracked output"
    if track_detection:
...
def normal_prediction(model,frame,frame_number,confidence,iou_threshold):
    results = model.infer(frame, confidence)[0]
